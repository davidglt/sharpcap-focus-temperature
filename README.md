...
| `sharpcap_focus_state.json` | Last valid autofocus reference plus the fitted thermal model (TCF, slope, intercept). |
...
After each successful run the script writes `sharpcap_focus_state.json` (or the
path given to `--output-state-json`).  This file is the **single source of truth**
for the thermal model and the latest focus reference:
...
Basic example (uses default SharpCap log path on Windows):
...
| `--output-csv` | `sharpcap_final_focus.csv` | Output CSV file path. |
| `--output-state-json` | `sharpcap_focus_state.json` | Output JSON file with last valid autofocus reference and regression model. |
| `--min-position` | `24000` | Minimum focuser position to keep. |
| `--max-position` | `27000` | Maximum focuser position to keep. |
| `--x-min` | `24000` | Minimum X axis limit. |
| `--x-max` | `27000` | Maximum X axis limit. |
...
3. Inspect `sharpcap_final_focus.csv` and `sharpcap_focus_temperature.png`.
4. Review `k`, `TCF`, and the focus prediction for the temperature of interest.
5. Point your nightly sequencer at `sharpcap_focus_state.json` to apply
   temperature-based focus compensation without re-parsing logs.
